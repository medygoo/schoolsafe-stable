...
-- A5.1 unit 08: same A5.0 bootstrap contract, plus exact projection of
...
begin
  -- A51_CURRENT_PROJECTION: scoped navigation metadata, never an action grant.
  ...
  with current_roles as materialized (
    select r.id,r.code,pr.starts_at,pr.ends_at from iam.profile_roles pr
    ...
    select gs.grant_id,gs.scope_code,gs.target_id,gs.starts_at,gs.ends_at from iam.grant_scopes gs
    ...
    select g.id,g.effect,p.code,greatest(g.starts_at,r.starts_at) as starts_at,
      least(g.ends_at,r.ends_at) as ends_at from iam.role_permission_grants g
    ...
    select es.exception_id,es.scope_code,es.target_id,es.starts_at,es.ends_at from iam.exception_scopes es
    join iam.profile_permission_exceptions e on e.id=es.exception_id and e.school_id=es.school_id
    where es.school_id=v_school and e.profile_id=v_profile and es.is_active and es.starts_at<=now() and (es.ends_at is null or es.ends_at>=now())
    ...
    select g.code,'role'::text as source,g.id::text as origin_id,s.scope_code,s.target_id,
      c.condition_code,c.condition_params,greatest(g.starts_at,s.starts_at) as starts_at,
      least(g.ends_at,s.ends_at) as ends_at
    ...
    select e.code,'exception'::text as source,e.id::text as origin_id,s.scope_code,s.target_id,
      e.condition_code,e.condition_params,greatest(e.starts_at,s.starts_at),least(e.expires_at,s.ends_at)
    from current_exceptions e
    join current_exception_scopes s on s.exception_id=e.id
    where e.effect='deny'
  ), global_denied_codes as materialized (
    -- Only unconditional school/none in THIS school removes a whole capability.
    -- IAM scope_matches ignores target_id for these two scopes.
    -- Teacher scopes in an origin still require class AND subject in IAM.
    select distinct d.code from denied_rules d
    where d.scope_code in ('school','none') and d.condition_code is null
      and not exists (select 1 from denied_rules other
        where other.source=d.source and other.origin_id=d.origin_id
          and other.scope_code in ('assigned_classes','assigned_subjects'))
  ), allowed_codes as (
    ...
    where g.effect='allow' and not exists(select 1 from global_denied_codes d where d.code=g.code)
    ...
    where e.effect='allow' and not exists(select 1 from global_denied_codes d where d.code=e.code)
  ...
  select jsonb_build_object(
    ...
    'permissions',coalesce((select jsonb_agg(a.code order by a.code) from allowed_codes a
      where not exists(select 1 from global_denied_codes d where d.code=a.code)),'[]'::jsonb),
