...
begin
  perform iam.require_access('school.student.create',null,p_student_id);
  if p_subject_type <> 'student' or p_student_id is null then
    raise check_violation using message='Only a verified student mapping is supported';
  end if;
  if not exists(select 1 from app.students where school_id=v_school_id and id=p_student_id) then
    raise foreign_key_violation using message='Student outside active school';
  end if;

  select * into v_device from devicehub.devices
  where id = p_device_id and school_id = v_school_id;
  if not found then
    raise exception using errcode = '23503', message = 'Appareil introuvable dans cette école';
  end if;

  -- Réutiliser le mapping actif existant (jamais deux mappings actifs).
  select * into v_mapping
  from devicehub.device_subject_mappings
  where school_id = v_school_id and device_id = p_device_id and subject_type = p_subject_type
  ...
  if not found then
    ...
    values (v_school_id, p_device_id, p_subject_type, p_student_id, pg_catalog.gen_random_uuid()::text, 1, 'pending', 1)
  ...
  where school_id = v_school_id and id = v_mapping.id;
...
begin
  ...
  -- Serialize deliveries for a device, including concurrent retries.
  if p_device_id is not null then
  ...
  if p_device_id is not null then
    update devicehub.devices set last_seen_at = now() where school_id = v_school_id and id = p_device_id;
  end if;
...
begin
  ...
  loop
    ...
    where school_id = v_school_id and id = v_row.id;
...
grant execute on function api.device_register(text,text,text,text,text,text,text) to schoolsafe_api;
grant execute on function api.device_mapping_ensure(uuid,uuid,text,text) to schoolsafe_api;
grant execute on function api.device_event_ingest(uuid,text,text,text,text,text,timestamptz,jsonb) to schoolsafe_api;
grant execute on function api.device_sync_job_claim(integer) to schoolsafe_api;
commit;
