...
-- SchoolSafe LOT 5-R1: Documents Table (Canonical)
...
CREATE INDEX IF NOT EXISTS idx_documents_school_year_type ON app.documents(school_id, academic_year_id, type_code);
CREATE INDEX IF NOT EXISTS idx_documents_code ON app.documents(document_code);
...
CREATE OR REPLACE FUNCTION app.update_documents_timestamp()
RETURNS TRIGGER AS $$
BEGIN
...
END;
$$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_update_documents_timestamp ON app.documents;
CREATE TRIGGER trg_update_documents_timestamp
    BEFORE UPDATE ON app.documents
    FOR EACH ROW
    EXECUTE FUNCTION app.update_documents_timestamp();
commit;